...
**Snapshot: v0.1.0.** `cargo run -p connector-cli -- build` compiles three providers and 25 curated
operations into 37 artifacts. The compiler, embedded Rust catalogue, JSON catalogue, and public
...
`diff` must finish with `37 artifacts up to date (3 providers checked)` for the current catalogue.
