...
**Snapshot: v0.20.0 + C-527 (re-measured 2026-08-11).** `cargo run -p connector-cli -- build` compiles **55 providers**, **66 services**
and **835 curated connector operations** — plus 53 events, 5 channel bindings and 77 Flux core entries
(29 operations, 43 node kinds, 5 capabilities) with 3 core JSON Schemas — into **1108 artifacts**. The
...
`diff` must finish with `1108 artifacts up to date (55 providers checked)` for the current catalogue.
