...
fn manifest(connector: &Connector, service: &str) -> Result<String> {
    ...
    struct Manifest<'a> {
        ...
        /// The bounded read a settings page invokes to verify this service's connection.
        #[serde(skip_serializing_if = "Option::is_none")]
        verify: Option<&'a str>,
        /// The complete service configuration surface. A host need not read provider TOML.
        #[serde(skip_serializing_if = "Vec::is_empty")]
        config: Vec<ManifestConfig<'a>>,
    ...
    let manifest = Manifest {
        ...
        verify: connector.verify.as_deref().filter(|id| {
            connector
                .operation(id)
                .is_some_and(|operation| operation.service == service)
        }),
...
/// One `[[config]]` block plus the level derived from its destination.
///
/// The declaration stays flattened so the manifest and `catalog.json` serialize the exact same IR
/// fields. `level` cannot be part of that authored shape: allowing an author to choose it would let
/// an OAuth client secret be presented to every tenant instead of supplied once by the operator.
#[derive(Serialize)]
struct ManifestConfig<'a> {
    #[serde(flatten)]
    declaration: &'a ConfigField,
    level: connector_spec::Level,
}

fn manifest_config(field: &ConfigField) -> Result<ManifestConfig<'_>> {
...
///
/// The complete declaration now also appears under `[[config]]` (C-87). This stays as the indexed
/// compatibility view, addressed by `(kind, name)` — the same address the runtime configuration port
/// keys a stored value on — plus the field name and label a form needs to draw the row.
#[derive(Serialize)]
...
"#;
    ...
    /// The header's second line states the current arrangement — auth assembled by the host
    /// resolver, the manifest becoming a projection of the catalog artifact — and names no
    /// superseded story as pending. C-535 closed C-10 as superseded-never-implemented, so the
    /// comment promising it pointed every committed manifest at work that is never coming (C-542).
    #[test]
    fn the_manifest_header_states_the_current_arrangement_not_a_superseded_story() {
        let artifacts = emit(&load(&inputs(HAND_AUTHORED)).unwrap()).unwrap();
        let manifest = &default_unit(&artifacts).manifest;

        let mut lines = manifest.lines();
        assert_eq!(
            lines.next().expect("the manifest has a header"),
            format!("# Generated by {} — do not edit.", generator()),
        );
        assert_eq!(
            lines.next().expect("the header has a second line"),
            "# Auth is assembled by the host resolver (`connector-pack`); this manifest becomes \
             a projection of the catalog artifact (C-534).",
        );
        assert!(
            !manifest.contains("C-10"),
            "the header must not promise a superseded story:\n{manifest}"
        );
    }
