...
//! safe.**
//!
//! `check_write_metadata` refuses `idempotency = "idempotent"` on every authored write. What the
//! original story was really about is
...
//! hatch for 'safely repeatable'"*). It was permitted here all along, **with
...
//!
//! - an authored `conditional` write must **state its condition**, because flux's wording is "safe to
//!   repeat under *stated* conditions" and nothing was making anyone state them;
//! - the condition is refused where it means nothing — on an authored read, and beside
//!   an `idempotency` that is not `conditional`;
//! - `idempotent` on a write stays refused **unconditionally**, because that value licenses
...
//! emitter refusal with `if false` left the whole workspace green. Each layer's coverage came
...
use connector_spec::{
    provider, Connector, HttpMethod, Idempotency, Operation, OperationDirection, ParamSet,
    Provenance, Quirks, Risk, DEFAULT_SERVICE,
};

/// The smallest provider file that reaches these guards: one operation, with method and authored
/// direction deliberately supplied as independent facts.
fn provider_toml(
    method: &str,
    direction: &str,
    idempotency: &str,
...
fn a_conditional_write_that_states_no_condition_is_refused() {
    for method in ["POST", "PUT", "PATCH", "DELETE"] {
        let refusal = emit(&provider_toml(method, "write", "conditional", None))
            .expect_err("an authored `conditional` write with no stated condition must be refused");
        assert!(
...
fn a_post_declaring_idempotent_is_still_refused_with_or_without_a_condition() {
    for condition in [None, Some("purging an already-purged cache is a no-op")] {
        let refusal = emit(&provider_toml("POST", "write", "idempotent", condition)).expect_err(
            "an authored write claiming `idempotent` must be refused however it is dressed",
        );
        assert!(
...
fn a_patch_declaring_idempotent_is_still_refused() {
    let refusal = emit(&provider_toml("PATCH", "write", "idempotent", None))
        .expect_err("an authored write claiming `idempotent` must be refused");
...
fn a_condition_that_says_nothing_does_not_unlock_the_claim() {
    for shrug in ["", "   ", "yes", "idempotent", "see above"] {
        let refusal = emit(&provider_toml("POST", "write", "conditional", Some(shrug)))
...
/// The field is refused on an authored read, independently of transport method.
#[test]
fn a_condition_is_refused_on_an_authored_read() {
    for method in ["GET", "HEAD", "OPTIONS"] {
        ...
        .expect_err("`repeatable_because` on an authored read must be refused");
        assert!(
            refusal.contains("repeatable_because"),
...
/// because every case reaching it came in through the loader, which had already refused.
///
...
fn the_emitter_refuses_an_in_memory_ir() {
    let cases: [(&str, Operation); 4] = [
        (
            "a condition on an authored read",
            operation(
                HttpMethod::Get,
        ...
        (
            "an authored conditional write stating no condition",
            operation(
                HttpMethod::Post,
                OperationDirection::Write,
        ...
        (
            "an authored conditional write whose condition says nothing",
            operation(
                HttpMethod::Post,
                OperationDirection::Write,
                Idempotency::Conditional,
