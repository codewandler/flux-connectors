...
pub fn emit_graph(connector: &Connector, graph: &Graph) -> Result<String> {
    let text = flux_lang::format::format_composite_op(&Lowering::new(connector, graph)?.lower()?);
...
impl<'a> Lowering<'a> {
    ...
    fn lower(&self) -> Result<CompositeOpDecl> {
        let mut body = self.emit_region(None)?;
    ...
    fn emit_region(&self, region: Option<&str>) -> Result<Vec<Node>> {
        let mut out = Vec::new();
    ...
    fn emit_node(&self, node: &GraphNode, out: &mut Vec<Node>) -> Result<()> {
        ...
        match &node.kind {
            ...
            NodeKind::Gate { condition } => {
                ...
                out.push(Node::When {
                    cond: Box::new(cond),
                    then: self.emit_region(Some(&node.id))?,
            ...
            NodeKind::Approval { message, risk } => out.push(Node::Confirm {
                message: message.clone(),
                risk: Some(risk_tag(*risk).to_string()),
                body: self.emit_region(Some(&node.id))?,
            }),
            ...
            NodeKind::Retry {
                ...
                let mut body = self.emit_region(Some(&node.id))?;
            ...
            NodeKind::Throttle { max, window_ms } => out.push(throttle_node(
                &self.graph.name,
                &node.id,
                *max,
                *window_ms,
                self.emit_region(Some(&node.id))?,
            )),
