...
fn without_a_vault_the_live_leg_is_skipped_and_never_reported_as_a_pass() {
    // The live leg's own name, as libtest spells it. A rename that missed this one leaves the filter
    // matching nothing, which reports `1 filtered out` and fails below rather than passing.
    const LIVE_LEG: &str = "the_http_transport_round_trips_against_a_live_dev_server";
