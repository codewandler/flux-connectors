...
// that touches `STORE_ENV` or `XDG_DATA_HOME` holds `env_lock` across the set-then-construct
// window.
//
// This was a private copy of `tests/support`'s lock while every test file was its own binary —
// the `static` was per process either way, and importing the shared one pulled in an identity
// provider this file has no use for. C-533 put all of this crate's tests in **one** process, so a
// private lock stopped excluding anything: `persistence.rs` mutates `XDG_DATA_HOME` under the
// shared lock, and two locks over one variable is no lock at all. It must be the shared one now.
