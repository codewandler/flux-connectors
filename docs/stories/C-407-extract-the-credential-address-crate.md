...
- [x] `credential` moves to its own crate; `connector-spec` and `connector-secrets` both depend on it.
- [x] `connector-secrets` re-exports from the new crate, so `use connector_secrets::CredentialRef` is
...
- [x] `connector-spec` is removed from the publish closure, and `scripts/publish-crates-io.sh` derives
      the new order from the manifests rather than a hand-written list.
- [x] **Failing-first test** — or, where a test cannot express it, a check: the published closure
...
- 2026-08-01 — **Landed.** `crates/connector-address` (`codewandler-connector-address`) holds
  `address` and `credential`, moved with `git mv` so the history follows them.

  **`address.rs` travelled with `credential.rs`, and it had to.** `CredentialRef::build` calls
  `validate_authority`, `validate_service_name` and `validate_member_name`, and
  `CredentialRef::pid` returns a `Pid` — all of them `address`'s. Leaving `address` in
  `connector-spec` would have made the new crate depend on the compiler, which is the cycle the
  extraction exists to break. They are also one grammar: one reserved `DEFAULT_SERVICE`, one
  elision rule, one character class.

  `DEFAULT_SERVICE` went with them for the same reason — both grammars are written against it, and
  a copy on either side of the crate boundary can drift. `connector_spec::ir` re-exports it, so
  `crate::ir::DEFAULT_SERVICE`, `crate::DEFAULT_SERVICE` and `connector_spec::DEFAULT_SERVICE` all
  still resolve.

  **The edge inverted rather than vanished.** `connector-spec` depends on `connector-address` and
  re-exports both modules whole (`pub use connector_address::{address, credential, …}`), so
  `connector_spec::address::Gid` and `connector_spec::credential::CredentialRef` are unchanged —
  `crates/connector-spec/tests/credential_paths.rs` and `service_partition.rs` compile untouched,
  which is the evidence for that.

  One deliberate API change in `connector-spec`: `Error::InvalidAddress` is now
  `InvalidAddress(#[from] AddressError)` rather than a struct variant. The parsers moved with the
  grammar they enforce, and a second unconstructible spelling of the same failure would be two
  names for one thing. `Display` is `#[error(transparent)]`, so the rendered message is byte-identical.

  Closure, before → after (`scripts/publish-crates-io.sh --print-order`, which derives it):
  `catalog, spec, secrets, pack` → `address, catalog, secrets, pack`.

  The property is pinned by `crates/connector-cli/tests/publish_closure.rs`:
  `no_machinery_crate_is_published` asserts no crate in the derived closure is machinery, and
  `every_member_is_classified_as_machinery_or_not` keeps that list total so a future emitter cannot
  arrive unclassified. `dependency_fence.rs` classifies the new crate as a compiler crate — that
  list is about *sockets*, not publishing, which is why it already contained the published
  `connector-catalog` and now says so.

  `cargo run -p connector-cli -- diff`: 558 artifacts up to date. No generated byte moved.
