...
- [x] A failing-first test enumerates the current nine `spec_source = null` operations and requires
      zero after the change.
- [x] The seven Support operations select official counterparts despite `.json` path and response-
...
- [x] The three prior write variants are represented honestly from `UpdateTicket` without duplicate-
      selector aliases or hand-authored wire surfaces.
- [x] The Messaging response cycle is handled by a general bounded-schema policy with a regression
      test, allowing `PostMessage` and `ListMessages` to select from the official document.
- [x] Zendesk's scoped build/diff, rehearsals, provenance checks and repository gates pass with all
...
- 2026-08-02: implementation now measures 35 operations, all spec-sourced: Support 19, Help Center
  7 and Messaging 9. The three former update variants collapse to the one vendor `UpdateTicket`,
  and response-only recursion is bounded without changing the fail-closed request policy.
- 2026-08-02: done after focused Zendesk rehearsals, provenance/spec selection checks and the full
  workspace no-fail-fast suite passed.
