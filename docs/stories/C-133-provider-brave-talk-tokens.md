...
- **2026-07-31 — dispatched, and parked before `providers/brave.toml` was written.** No provider file
  exists, so **no whole-catalogue artifact is stale and there was nothing to resolve at integration**
  — not eight red tests, not nine, zero. (Coordinator verification, kept from the integration branch;
  the implementor's fuller account with its file:line evidence follows.)
