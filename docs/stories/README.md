...
### Provider Fleet 2
- [C-161 — Ship the Okta connector](C-161-provider-okta.md) · Spec · UNBLOCKED by C-184, which built the prefix axis this story measured as missing. Okta now spells `scheme = { header = { name = \"Authorization\", prefix = \"SSWS \" } }`. The `## Progress` findings stand as the record of why the axis exists; what remains is the connector itself — no provider TOML was ever shipped for the probe
