...
- [C-164 — Ship the Algolia connector](C-164-provider-algolia.md) · Spec · blocked — measured, not predicted. `ConfigField::binds` parses to exactly one of five destinations (crates/connector-spec/src/config.rs:178-202,239-267) and none is a request header; the one route that reaches a header — an `[[auth]]` credential — forces `secret = true` on whatever config field binds it (provider.rs:609-629), and the application id is not a secret. The hostname and the header cannot share one declared value; filed as a finding for C-187.
- [C-191 — Publish the explorer components as an npm package — C-142's deferral condition is met](C-191-publish-the-explorer-components.md) · Surfaces · BLOCKED on C-205 (the web gate is red on main) and C-158 (this story's own Acceptance requires it first). C-142 deferred extraction 'until a second consumer exists'. babelforce's ai-agent-platform console is that second consumer, filed 2026-07-31. The deferral was conditional and the condition is now true
