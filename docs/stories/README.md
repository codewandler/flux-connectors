...
- [C-185 — A request body cannot contain an array, so an envelope-shaped vendor cannot be addressed](C-185-body-arrays.md) · Codegen · found by C-168: BodyNode builds nested objects via `wire` and never arrays. SendGrid's mail send needs personalizations[].to[] and was excluded rather than faked — the catalogue now has an email provider that cannot send email
