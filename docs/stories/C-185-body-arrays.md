...
note: "found by C-168: BodyNode builds nested objects via `wire` and never arrays. SendGrid's mail send needs personalizations[].to[] and was excluded rather than faked — the catalogue now has an email provider that cannot send email"
---
...
(`crates/connector-flux/src/op.rs`) composes a nested **object** from a dotted `wire` path. There is no
spelling that produces a JSON **array** at any depth.
...
- **Cloudflare** ([C-169](C-169-provider-cloudflare.md)) — cache purge takes `files[]`.
