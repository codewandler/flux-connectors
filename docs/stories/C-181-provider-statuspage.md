...
- [ ] `providers/statuspage.toml`, hand-authored and **curated** — a small set of operations this pipeline
...
- [ ] `crates/connector-flux/tests/statuspage_connector.rs` — a per-provider contract test asserting the
      thing *this* connector is about (see the archetype above), not that the file parses.
- [ ] **Failing-first test:** the contract test must fail before `providers/statuspage.toml` exists.
- [ ] The scoped gate is green: `build --provider statuspage`, `diff --provider statuspage` reporting no drift,
