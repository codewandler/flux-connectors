...
- [x] A failing-first vendored-spec test names the absent Asterisk source contract.
- [x] The exact 11 raw documents, licence, upstream tag/commit and hashes are committed under
      `specs/asterisk/` with a reproducible public-source vendor script.
- [x] A deterministic normalizer produces OpenAPI 3 from those bytes and refuses unknown source
      versions, document inventories, types, placements, duplicate operation ids, or unaccounted
      operations.
- [x] Tests measure exactly 109 source operations, emit exactly 108 REST operations, and identify
      `events.eventWebsocket` as the sole deferred operation.
- [x] Normalization preserves paths, methods, parameters, response models, descriptions and source
...
- 2026-08-02: committed the 11 exact upstream documents, provenance and deterministic normalizer;
  five scoped tests account for 109 source operations, 108 REST outputs and one deferred WebSocket.
